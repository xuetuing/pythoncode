...
class PreFilters:
    # 是否停牌 stocksinfo为get_price返回
    def suspendStatu(self, suspendLag, stocksInfo):
        unsuspend_index = stocksInfo.iloc[:,0]<1
        unsuspend_stock_ = list(stocksInfo[unsuspend_index].index)
        # 进一步筛选出最近suspendLag+1日未曾停牌的股票list
        unsuspend_stock = []
        for stock in unsuspend_stock_:
            if sum(attribute_history(stock,suspendLag+1,'1d',('paused'),skip_paused=False))[0]==0:
